...
function hexOrBuffer(valueInput, name) {
    ...
    if (!Buffer.isBuffer(value)) {
        if (!isHexString(value)) {
            const error = new Error(name ? (`[neatjs-abi] invalid ${name}`) : '[neatjs-abi] invalid hex or buffer, must be a prefixed alphanumeric even length hex string');
            error.reason = '[neatjs-abi] invalid hex string, hex must be prefixed and alphanumeric (e.g. 0x023..)';
            error.value = value;
            throw error;
        }
...
function getKeys(params, key, allowEmpty) {
    ...
    if (!Array.isArray(params)) { throw new Error(`[neatjs-abi] while getting keys, invalid params value ${JSON.stringify(params)}`); }
    ...
    for (var i = 0; i < params.length; i++) { // eslint-disable-line
        ...
        if (allowEmpty && !value) {
            ...
            throw new Error('[neatjs-abi] while getKeys found invalid ABI data structure, type value not string');
        }
        result.push(value);
    }
...
function coderFixedBytes(length) {
    return {
        ...
        decode: function decodeFixedBytes(data, offset) {
            if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[neatjs-abi] while decoding fixed bytes, invalid bytes data length: ${length}`); }
...
const coderAddress = {
    encode: function encodeAddress(valueInput) {
        var value = valueInput; // eslint-disable-line
        var result = Buffer.alloc(32); // eslint-disable-line
        if (!isHexString(value, 20)) { throw new Error('[neatjs-abi] while encoding address, invalid address value, not alphanumeric 20 byte hex string'); }
        value = hexOrBuffer(value);
        result.fill(0);
    ...
    decode: function decodeAddress(data, offset) {
        ...
        if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[neatjs-abi] while decoding address data, invalid address data, invalid byte length ${data.length}`); }
        return {
            consumed: 32,
...
function decodeDynamicBytesHelper(data, offset) {
    if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[neatjs-abi] while decoding dynamic bytes data, invalid bytes length: ${data.length} should be less than ${offset + 32}`); }

    var length = uint256Coder.decode(data, offset).value; // eslint-disable-line
    length = length.toNumber();
    if (data.length !== 0 && data.length < offset + 32 + length) { throw new Error(`[neatjs-abi] while decoding dynamic bytes data, invalid bytes length: ${data.length} should be less than ${offset + 32 + length}`); }
...
function coderArray(coder, lengthInput) {
    return {
        encode: function encodeArray(value) {
            ...
            if (!Array.isArray(value)) { throw new Error('[neatjs-abi] while encoding array, invalid array data, not type Object (Array)'); }
            ...
            if (length !== value.length) { throw new Error(`[neatjs-abi] while encoding array, size mismatch array length ${length} does not equal ${value.length}`); }
...
function getParamCoder(typeInput) {
    ...
    const invalidTypeErrorMessage = `[neatjs-abi] while getting param coder (getParamCoder) type value ${JSON.stringify(type)} is either invalid or unsupported by neatjs-abi.`;
        ...
        switch (prefix) {
            ...
            case 'bytes':
                ...
                if (part[3]) {
                    ...
                    if (size === 0 || size > 32) {
                        throw new Error(`[neatjs-abi] while getting param coder for prefix bytes, invalid type ${type}, size ${size} should be 0 or greater than 32`);
                    }
...
const unitMap = {
    ...
    'neat':        '1000000000000000000', // eslint-disable-line
};
 ...
 * @param {String} unit the unit to convert to, default neat
 * @returns {BigNumber} value of the unit (in Wei)
 * @throws error if the unit is not correct:w
 */
function getValueOfUnit(unitInput) {
    const unit = unitInput ? unitInput.toLowerCase() : 'neat';
    var unitValue = unitMap[unit]; // eslint-disable-line

    if (typeof unitValue !== 'string') {
        throw new Error(`[neatjs-unit] the unit provided ${unitInput} doesn't exists, please use the one of the following units ${JSON.stringify(unitMap, null, 2)}`);
    }
...
function toWei(neatInput, unit) {
    var neat = numberToString(neatInput); // eslint-disable-line
    const base = getValueOfUnit(unit);
    const baseLength = unitMap[unit].length - 1 || 1;

    // Is it negative?
    var negative = (neat.substring(0, 1) === '-'); // eslint-disable-line
    if (negative) {
        neat = neat.substring(1);
    }

    if (neat === '.') { throw new Error(`[neatjs-unit] while converting number ${neatInput} to wei, invalid value`); }

    // Split it into a whole and fractional part
    var comps = neat.split('.'); // eslint-disable-line
    if (comps.length > 2) { throw new Error(`[neatjs-unit] while converting number ${neatInput} to wei,  too many decimal points`); }

    var whole = comps[0], fraction = comps[1]; // eslint-disable-line

    if (!whole) { whole = '0'; }
    if (!fraction) { fraction = '0'; }
    if (fraction.length > baseLength) { throw new Error(`[neatjs-unit] while converting number ${neatInput} to wei, too many decimal places`); }
...
var fromNio = function(number) {
    ...
    return isBN(number) ? toWei(number, "neat") : toWei(number, "neat").toString(10);
};
...
var toNio = function(number) {
    ...
    return isBN(number) ? fromWei(number, 'neat') : fromWei(number, 'neat').toString(10);
};
...
var toChecksumAddress = function (address) {
    ...
    if(!/^(0x)?[0-9a-f]{40}$/i.test(address))
        throw new Error('Given address "'+ address +'" is not a valid HEX address.');
